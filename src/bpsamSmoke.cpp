...
cbs::BPSAM::Ptr makeSam(cbs::AgentId id) {
  ...
  params.gbp_update_params.enable_soft_reset = true;
...
void addPoseChain(cbs::BPSAM &sam, cbs::AgentId robot_id, size_t n_poses) {
  ...
  for (size_t i = 1; i < n_poses; ++i) {
    ...
    values.insert(key_curr,
                  Pose3(Rot3::RzRyRx(0.0, 0.0, 0.0), Point3(i, 0.0, 0.0)));
  }
...
std::map<Key, std::vector<std::pair<cbs::AgentId, gbp::Gaussian>>>
buildBeliefs(cbs::BPSAM &source, cbs::AgentId source_id, cbs::AgentId target_id,
             size_t n_poses) {
